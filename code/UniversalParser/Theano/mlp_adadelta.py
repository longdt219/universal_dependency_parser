...
class MLP(object):
    ...
    def __init__(self, rng,  input, E, W1,B1, W2):
        ...
        self._accudeltas =   [build_shared_zeros(t.shape.eval(),'accudelta') for t in self.params]

    
def test_mlp(learning_rate=0.95, L2_reg=0.00000001, n_epochs=200,
   ...
   """
    ...
    print (" Adadelta Epsilon  : " + str(ada_epsilon))
    ...
    for accugrad, accudelta, param, gparam in zip(classifier._accugrads,classifier._accudeltas, classifier.params, gparams):
            # c.f. Algorithm 1 in the Adadelta paper (Zeiler 2012)
            agrad = _rho * accugrad + (1 - _rho) * gparam * gparam
            dx = - T.sqrt((accudelta + _eps)
                          / (agrad + _eps)) * gparam
            updates[accudelta] = (_rho * accudelta + (1 - _rho) * dx * dx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c','--decay', help='Decay rate (p = 0.95) of the system', required=True, type=float)
    parser.add_argument('-r','--reg', help='regularlization params', required=True, type=float)
    parser.add_argument('-e','--eps', help='adadelta epsilon params', required=True, type=float)
    ...
    test_mlp(learning_rate = learn_rate, L2_reg = reg_param, n_epochs = 200, dataset = input_file, 
