...
from utils import load_data, save_model, load_mapping_matrix
...
class MLP(object):
    ...
    def __init__(self, rng,  input, refInput,  E, W1,B1, W2, refE, refW1, refB1, refW2, mapping):
        ...
        self.refHiddenLayer = HiddenLayer(rng, 
            input=self.refEmbeddingLayer.output,
            n_in = ref_n_in1,
            n_out = ref_n_out1)

        self.refLogRegressionLayer = SoftMaxLayer(rng, 
            input=self.refHiddenLayer.output,
            n_in = ref_n_in2, 
            n_out = ref_n_out2,
        )
        ...
        self.refNegative_log_likelihood = (
            self.refLogRegressionLayer.negative_log_likelihood
        )
        
        # same holds for the function computing the number of errors
        self.errors = self.logRegressionLayer.errors
        self.params = (self.embeddingLayer.params +  self.hiddenLayer.params  + self.logRegressionLayer.params + 
                       self.refEmbeddingLayer.params +  self.refHiddenLayer.params + self.refLogRegressionLayer.params)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-j','--reg_join', help='regularlization params for joinly train', required=True, type=float)
    ...
    test_mlp(learning_rate = learn_rate, L2_reg = reg_param, n_epochs = 100000000, dataset = input_file, ref_dataset = ref_file,  
                  batch_size = b_size, max_iter = iteration, output=output_file, validation_freq = validation_freq, ada_epsilon = eps_param, L2_reg_tie=reg_join,
                  map_file = map_file)
