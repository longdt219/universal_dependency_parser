...
from utils import load_data, save_model, load_mapping_matrix, dropout_from_layer, load_translation_vector, save_embedding
...
class MLP(object):
    ...
    def __init__(self, rng,  input, refInput,  E, W1,B1, W2, refE, refW1, refB1, refW2, mapping, drop_out_rate = 0.5, drop_out_embedding_rate = 0.2, ref_tras_idx = None,
        tras_idx =  None ):
...
#                       (self.embeddingLayer.E ** 2).sum() +  
 #                             (self.refEmbeddingLayer.E ** 2).sum() + 
        ...
        # Tieing together  the bilingual translation table  
        self.reg_L2_sqr = (
                                    ((self.embeddingLayer.E[tras_idx] - self.refEmbeddingLayer.E[ref_tras_idx]) ** 2).sum()
                                   )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rj','--regjoin', help='regularlization params for tying together Embedding', required=True, type=float)
    ...
    parser.add_argument('-bd','--bidict', help='Bilingual Dictionary File as the list of (source (en) ,target (sv)) ', required=True)
    ...
    test_mlp(learning_rate = learn_rate, L2_reg = reg_param, n_epochs = 100000000, dataset = input_file, ref_dataset = ref_file,  
                  ...
                  reg_join=reg_join, map_file = map_file, bidict_file  = bi_dict_file, english_file = english_file)
