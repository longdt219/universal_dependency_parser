 ...
 * @author Jon Gauthier
 */
public class DependencyParserTheano {
...
private String trainer; 
  ...
  DependencyParserTheano() {
    this(new Properties());
  }

  public DependencyParserTheano(Properties properties) {
...
//	        	  feature.add(x);
   ...
   */
  public static DependencyParserTheano loadFromModelFile(String modelFile) {
   ...
   */
  public static DependencyParserTheano loadFromModelFile(String modelFile, Properties extraProperties) {
    DependencyParserTheano parser = extraProperties == null ? new DependencyParserTheano() : new DependencyParserTheano(extraProperties);
   ...
   *                  training corpus
   */
  public void train(String trainFile, String devFile, String modelFile, String embedFile, String outputTheano, String outModelTheano) {
    ...
    String cmd = String.format("python ../../code/UniversalParser/Theano/mlp_adagrad.py --rate %s --reg %.10f --bsize %s "
    		+ "  --iter %s --input %s --output %s --valfreq %s --eps %.10f", config.adaAlpha, config.regParameter, config.batchSize, config.maxIter,
    		                                    outputTheano, outModelTheano, config.evalPerIter, config.adaEps);

    if (this.trainer.toLowerCase().trim().equals("adadelta")){
    	System.out.println(" Using trainer : ADADELTA ");
    	cmd = String.format("python ../../code/UniversalParser/Theano/mlp_adadelta.py --decay %s --reg %.10f --bsize %s "
    		+ "  --iter %s --input %s --output %s --valfreq %s --eps %.10f", config.adaDelta_momentum, config.regParameter, config.batchSize, config.maxIter,
    		                                    outputTheano, outModelTheano, config.evalPerIter, config.adaEps);    	
    }
    
    if (this.trainer.toLowerCase().trim().equals("dropout")){
    	System.out.println(" Using trainer : Adagrad + Dropout");
        cmd = String.format("python ../../code/UniversalParser/Theano/mlp_dropout.py --rate %s --reg %.10f --bsize %s "
        		+ "  --iter %s --input %s --output %s --valfreq %s --eps %.10f", config.adaAlpha, config.regParameter, config.batchSize, config.maxIter,
        		                                    outputTheano, outModelTheano, config.evalPerIter, config.adaEps);
    }
  ...
  public void train(String trainFile, String devFile, String modelFile) {
    train(trainFile, devFile, modelFile, null,null,null);
  }
 ...
 * @throws Exception 
  ...
  public static void main(String[] args) throws Exception {
    ...
    DependencyParserTheano parser = new DependencyParserTheano(props);
