...
public class Classifier {
  ...
  private double refRegParam ;
  private ParsingSystem system = null;
  private ParsingSystem refSystem = null;
  private HashMap<Integer, Boolean> InitializationIndex = null;
  private List<String> knownWords;
  private List<String> knownLabels;
  private List<String> refKnownLabels;
  private List<String> refKnownWords;
  private List<String> refKnownPos;
  private double refRegParamPOSARC;

  
  public void setRefClassifier(Classifier refClass) {
	this.refClassifier = refClass;
  }

  public void setRefRegParam(double refRegParam) {
	this.refRegParam = refRegParam;
  }
  public void setSystem(ParsingSystem system) {
	this.system = system;
  }
  public void setRefSystem(ParsingSystem refSystem) {
...
private List<String> knownPos;
  ...
  public Classifier(Config config, Dataset dataset, double[][] E, double[][] W1, double[] b1, double[][] W2,
    ...
    // Long Duong : print stack here 
    //System.out.println(" Num Label = " + numLabels);
...
//    }
  ...
  private class CostFunction implements ThreadsafeProcessor<Pair<Collection<Example>, FeedforwardParams>, Cost> {
    ...
    public Cost process(Pair<Collection<Example>, FeedforwardParams> input) {
      ...
      for (Example ex : examples) {
        ...
        // Long Duong : Here is the most important line of code, compute the cost function according to the soft-max 
        // ??? Another concern is why scores[i] - maxScore 
  ...
  public class Cost {
    ...
    // Long Duong : Remove the gradient of fixed parameters 
	public void removeGradientForEmbedding( HashMap<Integer, Boolean> initializationIndex) {
	      for (int i = 0; i < E.length; ++i) {
	    	  if (initializationIndex.containsKey(i)){
	    		  for (int j = 0; j < E[i].length; ++j) {
	    			  gradE[i][j] = 0; //Fix gradient of this parameter = 0 so that do not update 
	    		  }
	    	  }
	        }
    ...
    private void addL2Regularization(double regularizationWeight) {
      for (int i = 0; i < W1.length; ++i) {
        for (int j = 0; j < W1[i].length; ++j) {
    		// Long Duong : We only want to regularlize the code that doesn't involve embedding         	
        	if ((refClassifier==null) || (j < 18 * config.embeddingSize)) { 
            	cost += regularizationWeight * W1[i][j] * W1[i][j] / 2.0;
            	gradW1[i][j] += regularizationWeight * W1[i][j];
        	}
        	else {
        		cost += refRegParam *  Math.pow(W1[i][j] - refClassifier.getW1()[i][j],2) / 2.0;
        		gradW1[i][j] += refRegParam * (W1[i][j] - refClassifier.getW1()[i][j]);
        	}
        }
      }
      ...
      for (int i = 0; i < b1.length; ++i) {
    	  if (refClassifier ==null) {
    		  ...
    		  cost += refRegParam * Math.pow(b1[i] - refClassifier.getb1()[i], 2) / 2.0;
    		  gradb1[i] += refRegParam * (b1[i] - refClassifier.getb1()[i]);    		  
    	  }
      }

      for (int i = 0; i < W2.length; ++i) {
    	  // Get the reference label corresponding to i
    	  double[] refW2 = null; 
    	  if (refClassifier !=null){
    		  String transitionLabel = system.transitions.get(i);
    		  refW2 = this.getW2jFromReference(transitionLabel);
        	  if (refW2 == null){
        		  System.out.println(" Can't find reference W2 for transition :  " + transitionLabel );
        	  }
    	  }
    			  
    	  for (int j = 0; j < W2[i].length; ++j) {
        	if ( (refClassifier == null) || (refW2 == null)){
                cost += regularizationWeight * W2[i][j] * W2[i][j] / 2.0;
                gradW2[i][j] += regularizationWeight * W2[i][j];
        	}
        	else {
        		if (refW2 !=null) {
                    cost += refRegParam * Math.pow(W2[i][j] - refW2[j],2) / 2.0;
                    gradW2[i][j] += refRegParam * (W2[i][j] - refW2[j]);
        		}
        	}
        }
      }
      // Long Duong : how to match E[i,j] back to word/pos/label form 
      for (int i = 0; i < E.length; ++i) {
    	  double[] refE = null;
    	  ...
    	  if (refClassifier !=null){
    	    	 if ((i >= knownWords.size())  && (i < knownWords.size() + knownPos.size()) ){
    	    		 // This is POS 
    	    		 String posLabel = knownPos.get(i - knownWords.size());
    	    		 refE = this.getPOSEjFromReference(posLabel);
    	    		 if (refE ==null)
    	    			 System.out.println(" Cant find match for " + posLabel);
    	    	 }
    	    	 if (i >= knownWords.size() + knownPos.size()){
    	    		 // This is Arc
    	    		 String arcLabel = knownLabels.get(i - knownWords.size() - knownPos.size());
    	    		 refE = this.getArcEjFromReference(arcLabel);
    	    		 if (refE ==null)
    	    			 System.out.println(" Cant find match for " + arcLabel);
    	    	 }
    	  }
    	
        for (int j = 0; j < E[i].length; ++j) {
        	if ((refClassifier ==null) ||  (refE == null)){
                cost += regularizationWeight * E[i][j] * E[i][j] / 2.0;
                gradE[i][j] += regularizationWeight * E[i][j];
        	}
        	else {
                cost += refRegParamPOSARC * Math.pow(E[i][j]- refE[j] , 2) / 2.0;
                gradE[i][j] += refRegParamPOSARC * (E[i][j] - refE[j]) ;
        	}
        }
      }
    }
    ...
    // Long Duong 
    private double[] getW2jFromReference(String transitionLabel){
    	double[][] refW2 = refClassifier.getW2();
    	for (int i =0; i<refW2.length; i++){
    		String ref_label = refSystem.transitions.get(i);
    		if (ref_label.trim().equals(transitionLabel.trim())){
    			return refW2[i];
    		}
    	}
    	return null;
    ...
    private double[] getPOSEjFromReference(String posLabel){
    	double[][] refE = refClassifier.getE();
    	for (int i =refKnownWords.size(); i<refKnownWords.size() + refKnownPos.size(); i++){
    		String ref_label = refKnownPos.get(i - refKnownWords.size());
    		if (ref_label.trim().equals(posLabel.trim())){
    			return refE[i];
    		}
    	}
    	return null;
    ...
    private double[] getArcEjFromReference(String arcLabel){
    	double[][] refE = refClassifier.getE();
    	for (int i =refKnownWords.size() + refKnownPos.size(); i<refKnownWords.size() + refKnownPos.size() + refKnownLabels.size(); i++){
    		String ref_label = refKnownLabels.get(i - refKnownWords.size() - refKnownPos.size());
    		if (ref_label.trim().equals(arcLabel.trim())){
    			return refE[i];
    		}
    	}
    	return null;
  ...
  public Cost computeCostFunction(int batchSize, double regParameter, double dropOutProb) {
    ...
    // Long Duong : Add regularlization here with the lambda = regParameter 
    cost.addL2Regularization(regParameter);

    // Long Duong : If the initializationIndex !=null, don't update the item where we know that it is initialized
    if (this.InitializationIndex !=null){
    	// remove the index of Embedding 
    	cost.removeGradientForEmbedding(this.InitializationIndex);
    }
...
public void setInitializationIndex(HashMap<Integer, Boolean> _initializationIndex) {
	this.InitializationIndex = _initializationIndex;
}

// Long Duong : Set the labels so that we can map back from E[i] to it corresponding labels  
public void setEandRefE(List<String> knownWords, List<String> knownPos,
		List<String> knownLabels, List<String> refKnownWords,
		List<String> refKnownPos, List<String> refKnownLabels) {
	this.knownWords = knownWords;
	this.knownPos = knownPos; 
	this.knownLabels = knownLabels; 
	this.refKnownWords = refKnownWords; 
	this.refKnownPos = refKnownPos ; 
	this.refKnownLabels = refKnownLabels;
}

public void setRefRegParamPOSARC(double d) {
	this.refRegParamPOSARC = d; 
}

// Long Duong : method to tell that training is done externally 
public void setTrainingFlag(boolean b) {
	this.isTraining = b; 
	
}

//public void setParams(double[][] e2, double[][] w1, double[] b, double[][] w2) {
