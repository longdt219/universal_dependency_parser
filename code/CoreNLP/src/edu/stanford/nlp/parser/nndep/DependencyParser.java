...
private String out_file = ""; 
  ...
  public void train(String trainFile, String devFile, String modelFile, String embedFile) {
     ...
     */
    double bestUAS = 0;
    // Long Duong : Set the reference for clasisfier 
    classifier.setRefClassifier(refClassifier);
    classifier.setRefRegParam(refRegulalizationParam);
    classifier.setRefRegParamPOSARC(refRegPOSARC);
    classifier.setSystem(system);
    classifier.setRefSystem(refSystem);
    classifier.setEandRefE(knownWords,knownPos, knownLabels, refKnownWords, refKnownPos, refKnownLabels);
    ...
    for (int iter = 0; iter < config.maxIter; ++iter) {
      ...
      if (devFile != null && iter % config.evalPerIter == 0) {
        ...
        if (config.saveIntermediate && uas > bestUAS) {
          System.out.printf("Exceeds best previous UAS of %f. Saving model file..%n", bestUAS);
      ...
      if (config.clearGradientsPerIter > 0 && iter % config.clearGradientsPerIter == 0) {
        System.out.println("Clearing gradient histories..");
    ...
    if (devFile != null) {
      ...
      if (uas > bestUAS) {
        System.out.printf("Final model UAS: %f%n", uas);
        System.out.printf("Exceeds best previous UAS of %f. Saving model file..%n", bestUAS);
  ...
  private void setupClassifierForTraining(List<CoreMap> trainSents, List<DependencyTree> trainTrees, String embedFile) {
    ...
    if (this.fixInitialization !=null)
